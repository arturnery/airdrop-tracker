...
import { listTasksByProject } from "@/db/queries/tasks";
...
export async function generateMetadata({
  ...
  return { title: `${projeto?.nome ?? "Projeto"} · airdrop-tracker` };
}
...
export default async function ProjetoPage({
  ...
  const tarefas = await listTasksByProject(projeto.id);
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description={
          [
            ...
            projeto.tgePrevisto ? `TGE previsto ${formatDateBr(projeto.tgePrevisto)}` : null,
          ]
            .filter(Boolean)
            .join(" · ") || undefined
        }
      ...
      <section aria-label="Indicadores do projeto" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          label="Aportado"
          accent="primary"
      ...
      <Tabs defaultValue="contas" className="mt-10">
        ...
        <TabsContent value="historico" className="mt-6">
              ...
              <table className="w-full min-w-160 text-sm">
                ...
                <tbody className="divide-border divide-y">
                  {projeto.historico.map((linha) => (
                    <tr key={linha.id} className="hover:bg-accent/40 transition-colors">
                      <td className="text-muted-foreground px-4 py-3 tabular">
          ...
          <p className="text-muted-foreground mt-3 text-xs">
            Linhas marcadas como <strong className="text-foreground">Saldo</strong> são
            fotos do saldo, não movimentação — por isso não entram na soma de capital
            aportado.
          </p>
        </TabsContent>
                ...
                <tbody className="divide-border divide-y">
                  {projeto.recebimentos.map((claim) => (
                    <tr key={claim.id}>
                      ...
                      <td className="px-4 py-3 text-right tabular">{claim.quantidade}</td>
                      <td className="px-4 py-3 text-right">
            ...
            <div className="border-border rounded-lg border p-5">
              ...
              <dl className="space-y-3 text-sm">
                ...
                <div className="flex justify-between gap-4">
                  ...
                  <dd className="tabular">
                    {projeto.ultimaAtividade ? formatDateBr(projeto.ultimaAtividade) : "—"}
                  </dd>
                </div>
              </dl>
